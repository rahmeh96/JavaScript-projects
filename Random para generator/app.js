...
function generateParagraph(event){
...
text.forEach((item) => {
    ...
    if(value === 0)
            {
                content.innerHTML = `<p "class="para">${text[randomNumber]}</p>`
            }

else if (value >0 && value <= 9){
    content.innerHTML = `<p "class="para">${text.slice(0, value).join('<br></br>')}</p>`

} else {
    content.innerHTML = `<p "class="para">${text[0]}</p>`
...
form.addEventListener('submit', generateParagraph)